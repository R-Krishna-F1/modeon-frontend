    ...
    <div className="min-h-screen bg-canvas">
      <nav className="flex items-center justify-between px-10 py-5 border-b border-divider">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => navigate(ROUTES.REGISTER)} className="btn-gold">
            Get Started Free
          </button>
        </div>
      </nav>
      ...
      <section className="text-center py-28 px-10 max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <p className="text-lg text-taupe max-w-xl mx-auto mb-10 leading-relaxed">
            Modeon turns your wardrobe into a personal style system - AI-generated outfits, smarter shopping decisions,
            and a closet you will actually use.
          </p>
      ...
      <section className="py-20 px-10 max-w-4xl mx-auto">
        <h2 className="font-display text-4xl text-center text-ink mb-4">Free and Open Source</h2>
        <p className="text-muted text-center mb-12">
          Every Modeon feature is available to everyone now. No subscriptions, no feature locks.
        </p>
        <div className="card max-w-2xl mx-auto text-center">
          <p className="text-sm text-taupe leading-relaxed mb-6">
            Build outfits, compare shopping choices, run outfit checks, and explore styling tools without hitting a paywall.
          </p>
          <div className="flex gap-3 justify-center flex-wrap">
            <button onClick={() => navigate(ROUTES.PRICING)} className="btn-gold">
              Explore Features
            </button>
            <button onClick={() => navigate(ROUTES.REGISTER)} className="btn-secondary">
              Start Using Modeon
            </button>
          </div>
        </div>
      ...
      <footer className="border-t border-divider py-10 px-10 text-center">
        ...
        <p className="text-sm text-muted">&copy; {new Date().getFullYear()} Modeon. Dress better, waste less.</p>
      </footer>
    </div>
  );
}
